...
## Portrait-first corporate PDF layout

Current View PDF uses A4 Portrait by default. Explicit Landscape remains A4 Landscape, while Auto retains the existing geometry-based choice only when selected. The source desktop width is captured before print preparation and proportionally fitted once; the widget does not trigger a mobile/responsive reflow. **Horizontal Page Margin (mm)** defaults to 10 and applies the same left/right margin. **Vertical Page Margin (mm)** defaults to 12 and applies the same top/bottom margin. **Smart Page Breaks** defaults to Yes and uses clone-only geometry to keep headings with initial content, table/grid headers with their first row, and small bordered cards intact when they fit. Oversized panels remain splittable, and short sections are not forced onto separate pages.
